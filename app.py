...
def index():
	...
	words = {
		'start': ['', 'someone', 'change', 'to stop', 'depend'],
		'surface': ['', 'someone', 'end', 'to stop', 'depend'],
		'beneath': ['', 'something', 'end', 'to stop', 'depend'],
		'fail': ['', 'something', 'end', '', 'depend'],
		'multi': [', yet', 'something', 'end', '', 'end'],
		'final': [', yet', 'something', 'end', '', 'end']
	}[session['mode']]
...
"I don't want that."],
		description="""
<center><img src='/static/face.gif'></center>
...
<p>Anyway, I'm not posting because I want to win, or for fame, or even just for fun.</p>
<!-- I don't want that. -->
<!-- I don't want that. -->
<!-- I don't want that. -->
<!-- I don't want that. -->
...
<p>Anyway, after leaning the larger whiteboard against the wall and drawing a rainbow-scented pile of lack of artistic talent on it, I decided to try out a new USB drive.
...
<p>[screenshot of final code in Finder]</p>
...
<p>and use the app.</p>
<p>[screenshot of localhost in Safari; there's already a saved game named BENEATH]</p>
...
<p>I wish there was a way to enable multiple save files. Maybe then it would let me start over.</p>
"""},
		'multi': {'title': 'Walkthrough: The Second Save', 'description': """
<p>Hint: try getting yourself killed!</p>
"""},
		'final': {'title': 'Walkthrough: The Final Save', 'description': """
<p>Hint: try getting yourself killed again!</p>
"""}}
...
def play():
	if 'mode' in session:
		if session['mode'] == 'surface':
			return inflectr.inflect(render_template('play.html', title='The Surface', links=['<a href="/play">continue</a>', '<a href="/">return to main menu</a>'], description="""
...
<p>You may turn the lights on and off. [links]</p>
"""))

		if session['mode'] == 'beneath':
			return inflectr.inflect(render_template('play.html', title='Beneath the Surface', links=['<a href="/play">continue</a>', '<a href="/die">die</a>'], description="""
...
<p>You may turn the lights on and off. [links, but they don't work]</p>
"""))
...
def fail():
	...
	return inflectr.inflect(render_template('play.html',
		...
		description="""
<p>[an unskippable creepy cutscene of NPCs laughing at your corpse]</p>
		"""))
...
def cheat(code):
	if code == 'clear':
		session.clear()
		page = {'title': 'CLEAR', 'description': """
<p>You use the cheat code CLEAR.</p>
<p>You feel completely free.</p>
"""}

	elif code == 'multi':
		session['mode'] = 'multi'
		page = {'title': 'MULTI', 'description': """
<p>You use the cheat code MULTI.</p>
...
"""}
	...
	elif code == 'quit' and session['mode'] == 'surface':
		session['mode'] = 'beneath'
		session['deaths'] = 0
		page = {'title': 'QUIT', 'description': """
<p>You use the cheat code QUIT.</p>
...
def play_filename(filename):
	if filename == 'YOU_FAILED':
		return inflectr.inflect(render_template('play.html',
			...
			links=['<a href="/">I am a repeated failure.</a>'],
			description="""
<p>[an unskippable creepy cutscene of NPCs laughing at your corpse, last frame only]</p>
			"""))

	if filename == 'BENEATH':
		return inflectr.inflect(render_template('play.html',
			title='Far Beneath the Surface',
			links=['<a href="/play/BENEATH">continue</a>', '<a href="/die">die</a>'],
			description="""
<p>You are in a small, dark house. Brownish paint peels off in long strips from the decaying walls. The front door lock is rusted shut. The living room has been looted of all valuables that weren't nailed down, all valuables that were nailed down, and the nails themselves. Metal prices must be up again.</p>
<p>The stairway leading down to your bedroom looks wobbly but usable, as long as you're up to date on your tetanus shots.</p>
<p>You may turn the lights on and off. [links, but they don't work]</p>
"""))
	...
	if filename == 'DONT_FAIL_AGAIN':
		return inflectr.inflect(render_template('play.html',
			...
			links=['<a href="/">vote</a>', '<a href="/">THE END</a>', '<a href="/cheat/clear">start over</a>'],
			description="""
...
def final():
	return inflectr.inflect(render_template('play.html',
		...
		links=['<a href="/">back to main menu</a>'],
		description="""
<p>The Final Scene. Threats before enabling DONT_FAIL_AGAIN.</p>
		"""))
